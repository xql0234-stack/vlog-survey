...
填答流程包含生活型態測驗與影片觀賞，
系統將分配兩支旅遊 Vlog 影片，
每支影片觀看完後，請您分別回答相同的一組題目。

🔸 **本問卷採1～7分量表（1＝非常不同意，7＝非常同意）。題目中的選項及初始數字僅為系統預設，請依您的實際感受調整作答。**

🎁填問卷抽星巴克禮券！  
為感謝您的協助，凡完成本問卷者可參加抽獎，將隨機抽出**8位獲得星巴克禮券乙份**。  
若願意參加，請於問卷最後留下 **電子信箱**（僅作中獎通知用途，不會與問卷內容連結分析）。  
...
if "lifestyle" in st.session_state:
    ...
    st.markdown("### 🎁 抽獎聯絡方式")
    email = st.text_input(
    "若願意參加星巴克禮券抽獎，請留下您的電子信箱（僅作中獎通知用途，不會與問卷內容連結分析）：",
    ""
)
    # -----------------------------
    # 提交整份問卷（獨立區塊）
