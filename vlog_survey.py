...
job = st.selectbox("7. 您的職業：", [
    "農林漁牧礦業", "製造業", "服務業", "自由業", "商業", "電子資訊工程業",
    "專業人員(法律、醫療、藝術表演等)", "軍、警、公、教", "家管", "學生", "退休/待業中/無業", "其他"
])
...
for category, qs in questions.items():
    for q in qs:
        responses[q] = st.slider(f"{question_num}. {q}", 1, 7, 4, key=q)
        question_num += 1
...
if st.button("計算生活型態"):
    ...
    st.markdown("### 📺 第一支影片（符合生活型態）")
    st.video(matched_video)

    st.markdown("### 🎬 第二支影片（隨機分配）")
    ...
    with st.form("video_form"):
        st.header("第三部分：影片評價題")
        st.markdown("請依序觀看上面兩支影片，並回答以下題目：")

        matched_scores = {}
        random_scores = {}
        for i, q in enumerate(video_questions + youtuber_questions, start=1):
            matched_scores[f"影片1_Q{i}"] = st.slider(f"影片1 - {q}", 1, 7, 4, key=f"mv1_{i}")
        for i, q in enumerate(video_questions + youtuber_questions, start=1):
            random_scores[f"影片2_Q{i}"] = st.slider(f"影片2 - {q}", 1, 7, 4, key=f"mv2_{i}")

        submitted = st.form_submit_button("提交整份問卷")
